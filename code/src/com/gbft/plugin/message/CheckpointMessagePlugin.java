...
public class CheckpointMessagePlugin implements MessagePlugin {
        ...
        if (message.getMessageType() == FETCH) {
            ...
            if (message.getFetch().getIsRequest()) {
                ...
                if (checkpointNum < checkpointManager.getMinCheckpoint()
                        || checkpointManager.getCheckpoint(Pair.of((long) entity.getId(), checkpointNum)).getServiceState() == null) {
                    ...
                    checkpointManager.getCheckpoint(Pair.of((long) entity.getId(), checkpointNum)).getServiceState().getRecords()
    ...
    public boolean hasQuorum(long checkpointNum) {
        return tally.hasQuorum(Pair.of((long) entity.getId(), checkpointNum), 0, new QuorumId(CHECKPOINT, quorum));
    }
    ...
    private synchronized void processCheckpoints(long checkpointNum) {
        ...
        if (tally.hasQuorum(Pair.of((long) entity.getId(), checkpointNum), 0, new QuorumId(CHECKPOINT, quorum))) {
            // update h
            if (checkpointManager.getCheckpoint(Pair.of((long) entity.getId(), checkpointNum)).getServiceState() != null) {
                checkpointManager.setLowWaterMark(checkpointNum);
            }
            ...
            if (checkpointNum > checkpointManager.getLastStableCheckpoint()) {
                ...
                if (checkpointNum > checkpointManager.getLowWaterMark() + checkpointManager.lowHighGap
                        && !isFetching.get()) {
                    var targets = tally.getQuorumNodes(Pair.of((long) entity.getId(), checkpointNum), 0, new QuorumId(CHECKPOINT, quorum));
                    ...
                    checkpointDigest = tally.getQuorumDigest(Pair.of((long) entity.getId(), checkpointNum), 0);
                    isFetching.set(true);
        ...
        if (max > min + 3) {
            for (var num = min; max > num + 3; num++) {
                ...
                if (tally.hasQuorum(Pair.of((long) entity.getId(), num), 0, new QuorumId(CHECKPOINT, quorum))) {
                    checkpointManager.removeCheckpoint(Pair.of((long) entity.getId(), num));
                } else {
                    return;
                }
            }
